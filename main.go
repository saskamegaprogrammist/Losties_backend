...
func main() {
	...
	r.HandleFunc("/user/{id}/ad",  losties_handlers.GetAdsH().NewAd).Methods("POST")
	r.HandleFunc("/user/{id}/ads",  losties_handlers.GetAdsH().GetUserAds).Methods("GET")
	r.HandleFunc("/user/{id}/ads/number",  losties_handlers.GetAdsH().GetUserAdsNumber).Methods("GET")
	r.HandleFunc("/ad/{id}",  losties_handlers.GetAdsH().GetAd).Methods("GET")
	r.HandleFunc("/ad/{id}/pet",  losties_handlers.GetPetsH().NewPet).Methods("POST")
	r.HandleFunc("/ad/{id}/pet",  losties_handlers.GetPetsH().GetAdPet).Methods("GET")
	r.HandleFunc("/ads",  losties_handlers.GetAdsH().GetAds).Methods("GET")
	r.HandleFunc("/ad/{id}/pet",  losties_handlers.GetPetsH().NewPet).Methods("POST")
	r.HandleFunc("/ad/{id}/pic",  losties_handlers.GetPicH().NewAdPic).Methods("POST")
	r.HandleFunc("/ad/{id}/pic",  losties_handlers.GetPicH().AdPicGet).Methods("GET")
	...
	r.HandleFunc("/ad/{id}/coords",  losties_handlers.GetCoordsH().NewCoords).Methods("POST")
	r.HandleFunc("/ad/{id}/coords",  losties_handlers.GetCoordsH().GetAdCoords).Methods("GET")
	r.HandleFunc("/coords",  losties_handlers.GetCoordsH().GetCoords).Methods("GET")
	r.HandleFunc("/ad/{id}/comments",  losties_handlers.GetCommentsH().GetAdComments).Methods("GET")
	r.HandleFunc("/ad/{id}/comment",  losties_handlers.GetCommentsH().NewComment).Methods("POST")


	cors := handlers.CORS(handlers.AllowedOrigins([]string{"http://localhost:5000", "http://warm-chamber-57831.herokuapp.com", "https://warm-chamber-57831.herokuapp.com"}), handlers.AllowCredentials(), handlers.AllowedMethods([]string{"POST", "GET", "PUT", "DELETE"}))
